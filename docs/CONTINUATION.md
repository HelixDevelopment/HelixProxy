...
1. `git fetch --all --prune` on **`main`** (operator: all work on main); confirm
   HEAD `f96da56` (== `main` == origin; integrate any newer foreign commit per
   ...
   actionable: hermetic-harness H0-full** — build `wireguard-go` (rootless `go
   build`), swap the veth in `tests/vpn_lan/hermetic_netns_poc.sh` for a userspace
   WireGuard tunnel, then H1/H2 add real peer services + `HELIX_BRIDGE_MODE=hermetic`
   to promote the 16 operator-gated protocol tests to AUTONOMOUS (§11.4.52); design
   `docs/design/vpn_lan_access/hermetic_wg_test_harness.md`. Run only with host
   process-headroom (§12 — transient fork exhaustion observed 2026-07-02).
