...
**Revision:** 9
**Last modified:** 2026-07-01T16:20:00Z
**Status:** Active — **§11.4.126 autonomous hardening loop** (operator: "keep hardening, don't tag yet") **+ NEW VPN-LAN service-access feature (plan landed, Phase 0 in flight)**. **Rev-9 wave:** `8140d40` promoted the §11.4.169 **security-ACL live-deny SKIP→GREEN** — S1 now reads Squid's own access.log (`TCP_DENIED/403`+`HIER_NONE` = deny enforced + no upstream leak, §11.4.68/§11.4.69), replacing the ambiguous `000` client-code SKIP; RED teeth (allowed `:443`⇒`TCP_TUNNEL/HIER_DIRECT`⇒no false deny-PASS §11.4.115), wired into the standing suite (§11.4.135). Full standing suite GREEN **69 run / 63 pass / 6 skip / 0 fail**, all 4 security-guard lines PASS. §11.4.169 matrix now **10 PASS / 1 honest SKIP** (`docs/design/hardening/Status.md` Rev 4). `fe9d9a9` landed the **comprehensive VPN-LAN phased plan** (`docs/design/vpn_lan_access/PLAN.md` + `PLAN_Summary.md` Rev 1) — 5 deep-research streams (§11.4.150 cited), env-var-decoupled svord bridge (§11.4.28), SSRF-reconciled (§11.4.120), 12 phases with tasks/subtasks/evidence + honest-SKIP posture. **3 subagents in flight (§11.4.70/§11.4.103, conductor reviews+commits §11.4.142):** (A) Phase-0 bridge scaffold (`.env.example`+`tests/lib/svord_bridge.sh`+`scripts/svord_doctor.sh`+companion doc, local-stub honest-SKIP proof); (B) control-plane coverage push (measure-then-raise-lowest-hermetic-pkg, -race clean, go.mod unchanged); (C) Phase-8 Miracast §11.4.112 structural-impossibility verdict doc. **Data-plane health FACT (§11.4.7):** base proxy UP — real 204 endpoint (gstatic) returns 204 via `:53128`, both listeners accept TCP, squid+dante healthy 2h; a fake-domain (`cache.example`) probe's 503/000 was NOT a regression. HEAD `fe9d9a9` (== `main` == origin). Host 43%.
