...
`main` == github/origin/upstream == `3b73f02`.** The full feature-branch history
...
1. `git fetch --all --prune` on **`main`** (operator: all work on main); confirm
   HEAD `3b73f02` (== `main` == origin; integrate any newer foreign commit per
   ...
   the H0-full real kernel-WireGuard tunnel (`hermetic_wg_roundtrip.sh`) + the model-A
   protocol promotions over it are all landed + independently reviewed + wired into the
   standing suite as §11.4.135 guards (`test_vpn_lan_hermetic` in `tests/run-tests.sh`).
   **The clean zero-install promotion set is COMPLETE (§11.4.6): Cast-eureka + FTP +
   WebDAV + email** all run AUTONOMOUSLY over the tunnel (unmodified protocol tests,
   stdlib peers on `10.10.0.2`, golden-bad teeth, 3/3 deterministic). Remaining protocol
   legs are genuinely operator-gated (§11.4.122/§11.4.3): SFTP (no `sshd`), SMB/NFS (no
   ...
   **Top non-operator-gated actionable = the hermetic-hardening follow-up batch** (one
   separate, independently-reviewed change): (a) add an explicit wrong-destination
   negative (fetch to underlay `10.9.0.2` / wg-down MUST-fail) to each harness so the
   §11.4.111 reachability-as-proof claim is self-evidencing rather than structural-only;
   (b) make each sibling harness's wg-preflight accept `/usr/sbin/wg` (the email harness
   already does via `PATH+=:/usr/sbin`), so they PASS wherever wireguard-tools is
   installed, not only when `wg` is on `PATH`. Design
