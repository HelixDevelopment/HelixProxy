...
1. `git fetch --all --prune` on **`main`** (operator: all work on main); confirm
   HEAD `91af9c6` (== `main` == origin; integrate any newer foreign commit per
   ...
   **The underlay-sniff AF_PACKET non-leak differential (task #63) is LANDED on the WG
   ...
   FINDINGS §7 → Rev 4 (PLANNED → IMPLEMENTED). **Top non-operator-gated actionables now:**
   **(task #64, tiny)** a one-line Ethernet-ethertype guard on the sniff frame analyzer
   (reviewer nit — a VLAN tag would shift Eth/IPv4/UDP offsets; structurally impossible on
   the harness's own fresh veth, so correct-by-construction hardening + a crafted-pcap unit
   assertion); **(task #65, fan-out)** replicate the sniff differential on the 4 protocol
   harnesses (`hermetic_bridge_run.sh`/`_ftp_run.sh`/`_webdav_run.sh`/`_email_run.sh`), each
   with a per-protocol plaintext marker (eureka JSON / FTP listing / WebDAV 207 body / mail
   token) + its own load-bearing `SNIFF_MUT=plain`. Both: builder → independent §11.4.142
