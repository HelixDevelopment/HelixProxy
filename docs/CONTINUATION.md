...
1. `git fetch --all --prune` on **`main`** (operator: all work on main); confirm
   HEAD `b76065c` (== `main` == origin; integrate any newer foreign commit per
   §11.4.71, no force §11.4.113). The single canonical moment-valid resume file is
   `.remember/remember.md` (§11.4.131) — read it first. **Top non-operator-gated
   actionable: hermetic-harness H1/H2** — H0 (veth) + H0-full (real kernel-WireGuard
   tunnel) are DONE + proven (`tests/vpn_lan/hermetic_netns_poc.sh` +
   `hermetic_wg_roundtrip.sh`, both deterministic + golden-bad teeth). NEXT: run real
   peer services unprivileged (smbd/vsftpd/webdav/eureka JSON/mDNS Go responder) inside
   the peer netns + wire `HELIX_BRIDGE_MODE=hermetic` into `tests/lib/svord_bridge.sh`
   and invoke the protocol tests INSIDE the namespace (model A — the peer at 10.10.0.2
   is only reachable from within `unshare`), promoting the 16 operator-gated SKIPs to
   AUTONOMOUS (§11.4.52). Design `docs/design/vpn_lan_access/hermetic_wg_test_harness.md`
   Rev 2. Kernel-WG-in-userns confirmed (no build); host headroom healthy post-restart.
