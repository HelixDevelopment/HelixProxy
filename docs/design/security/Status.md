...
**Revision:** 3
**Last modified:** 2026-07-01T14:48:00Z
**Status:** Static config-security review of the proxy data plane (Squid forward proxy + Dante SOCKS5). 2 areas already HARDENED (Squid ACL chain, Squid TLS N/A-by-design); 4 Squid header/version-hygiene leaks were FIXED (`via`, `httpd_suppress_version_string`, `forwarded_for`, `visible_hostname`) — RED→GREEN proven (the `Via` leak is gone + the security test S3 gate now PASSes); the Dante SSRF/BIND exposure was also FIXED (`command: connect` + `socks block` for link-local/loopback/RFC1918 — RED→GREEN proven, dante-log-confirmed, security-test S4 gate PASS); 2 items remain TRACKED-for-operator because the safe fix carries a connectivity risk requiring an operator decision (Squid `dns_nameservers` DNS-leak in static mode; Dante client-side open-relay — `socksmethod none` + `client pass from:0.0.0.0/0`). One leak is CONFIRMED live (`via`); one is INFERRED from Squid defaults and fixed as defense-in-depth (`forwarded_for` / X-Forwarded-For) — the distinction is stated per §11.4.6, no guessing. **Rev 3:** the S3/S4 security guard (`tests/security/proxy_acl_security.sh`) is now WIRED into the standing regression suite (`tests/run-tests.sh` `test_security_guards`, §11.4.135) with GREEN + RED-polarity, so a header-hygiene or SSRF regression fails the build; and the S4 SSRF verdict was hardened (§11.4.142 independent-review finding) to require dante's own `block(N)` log line as the authoritative §11.4.69 sink-side signal rather than an elapsed-time heuristic (which was bluff-capable on a fast-refusing host).
