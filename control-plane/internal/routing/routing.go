...
// fail-closed block ONCE — the `http_access deny !tun_up` line is inserted before
// the base `allow localnet` (cmd/compiler places the `include` above the base
// allow rules). Dante has no `include` directive, so its routes are ADDITIVE BY
// CONCATENATION (base sockd.conf verbatim + appended route{} blocks) — cmd/compiler
// performs the concatenation; this package renders only the appended blocks.
package routing
...
#   include (§11.4.122): pulled into config/squid/squid.conf ABOVE the base allow
#   rules so ` + "`http_access deny !tun_up`" + ` is reached before ` + "`allow localnet`" + `.
